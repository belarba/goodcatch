...
- Key places in the script:
  ...
  - Rule cards (`CARDS`, practice purse net only for now): each practice map offers three face-up cards from `CARD_REF`, dealt when practice opens. The player must pick one before placing buoys; it locks on the first buoy (Clear unlocks). Every card must stay linear so the net remains an integer program. `node tools/solve-net.mjs cards [maps]` solves every card per map, keeps a trio whose best two cards finish within 10%, checks each optimum against the page's scoring and writes `CARD_REF` (`// @card-ref-start` … `// @card-ref-end`).
...
- Visual system: `DESIGN.md` (with `.impeccable/design.json`); product context: `PRODUCT.md`.
